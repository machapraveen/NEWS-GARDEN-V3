...
export default function ArticleDetail() {
  ...
  const handleAnalyze = async () => {
    ...
    setAiLoading(true);
    try {
      const result = await analyzeArticleFull(article);
      if (result.analysis) setAnalysis(result.analysis);
      if (result.credibility) setCredibility(result.credibility);
      setAiDone(true);
      toast({ title: "Analysis complete", description: "AI analysis results are now available." });
    } catch (err) {
      console.error('AI analysis failed:', err);
      toast({ title: "Analysis failed", description: "Could not complete AI analysis.", variant: "destructive" });
    } finally {
      setAiLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <header className="mb-8">
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            <Badge className={sentimentStyles[displaySentiment as keyof typeof sentimentStyles] || sentimentStyles.neutral}>
              {displaySentiment} &middot; {Math.round(displaySentimentScore * 100)}%
            </Badge>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* AI Summary — shown when analysis is done */}
            {aiDone && displaySummary && (
            ...
            {/* Named Entities — shown when analysis provides them */}
            {displayEntities.length > 0 && aiDone && (
              <Card className="bg-card border-border/50">
                <CardHeader className="pb-2">
          ...
          <div className="space-y-6">
            ...
            {/* Credibility Score — shown after analysis */}
            {aiDone && (
              <Card className="bg-card border-border/50">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-display flex items-center gap-2">
            ...
            {/* Fake News Detection — shown after analysis */}
            {aiDone && (
              <Card className="bg-card border-border/50">
                <CardHeader className="pb-2">
                      ...
                      <AccordionContent>
                        <div className="text-xs text-muted-foreground space-y-2">
                          <div className="flex justify-between">
                            ...
                            <span className="text-foreground">{Math.round(bertConf * 100)}%</span>
                          </div>
